...
<div class="flex flex-col items-center justify-center min-h-screen py-8">
    ...
    <h2 class="text-3xl font-bold mb-8 text-center">Form Pengaduan</h2>
    ...
    <form action="{{ route('ticket.store') }}" method="POST" enctype="multipart/form-data" 
        ...
        <div>
            ...
            <textarea name="message" required rows="6"
                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-vertical @error('message') border-red-500 @enderror">{{ old('message') }}</textarea>
            @error('message')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            <label class="font-semibold text-gray-700 block mb-2">Upload Gambar (Opsional)</label>
